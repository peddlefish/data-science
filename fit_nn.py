...
def main():
	...
	nn_traindata = traindata_by_visitnumber.drop(['TripType','Upc','ScanCount','DepartmentDescription','FinelineNumber'],1)
	nn = MLPClassifier(algorithm='l-bfgs', alpha=1e-5, hidden_layer_sizes=(5, 2), random_state=1)
	nn = nn.fit(nn_traindata, target)
	...
	nn_testdata = testdata_by_visitnumber.drop(['Upc','ScanCount','DepartmentDescription','FinelineNumber'],1)
	nn_testdata.fillna(0,inplace=True)
	result = nn.predict_proba(nn_testdata)
	print type(result)
	...
	result_csv2.to_csv('/Users/Fanghui/Desktop/walmart-classification/result_nn.csv',header=True, index=True,delimiter=',')
